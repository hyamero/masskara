import Image from "next/image";
import aboutMasskara from "public/images/about-masskara.jpg";

export const About = () => {
  return (
    <div id="about" className="min-h-screen bg-black px-32">
      <div className="overflow-hidden rounded-3xl ">
        <Image
          src={aboutMasskara}
          id="showcase-img"
          alt="Bonsai Showcase Image"
          className="w-full scale-125 rounded-md object-cover object-center"
        />
      </div>

      <p className="text-offwhite mt-20 w-5/6 font-spaceGrotesk text-5xl font-light leading-tight">
        The festival is known for its extravagant display of masks and vibrant
        costumes, reflecting the city&apos;s creativity and exuberance.
        It&apos;s a harmonious blend of diverse cultural elements, and each year
        in October, it comes alive with lively street dancing, captivating
        musical performances, and a dazzling beauty pageant.
      </p>
    </div>
  );
};
